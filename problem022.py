...
lettervalue = {chr(ord("A")+i):i+1 for i in range(0,27)}

def namescore(x):
    lst = [lettervalue[i] for i in x]
    return sum(lst)

name = open('Py_Euler_22.txt', 'r')
name = name.read()
name = name.replace('"','')
name = name.split(',')
name.sort()

lst = [namescore(name[i])*(i+1) for i in range(0,len(name))]
print(sum(lst))
